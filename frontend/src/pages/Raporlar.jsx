...
const TABS = [
  ...
  { id: 'daire_arac', label: 'Daire-Araç' },
...
export default function Raporlar() {
  ...
  const [daireArac, setDaireArac] = useState([]);
  ...
  async function loadDaireArac() {
    try {
      const { data } = await api.get('/kontroller/daire-arac', {
        params: { baslangic: filt.baslangic, bitis: filt.bitis },
      });
      setDaireArac(data.kayitlar);
    } catch (e) { toast.error(apiError(e)); }
  }
  ...
  useEffect(() => {
    ...
    else if (tab === 'daire_arac') loadDaireArac();
  ...
  function exportCsv() {
    ...
    } else if (tab === 'daire_arac') {
      const csv = toCSV(daireArac, [
        { key: 'daire_no', label: 'Daire', get: (r) => r.daire_no || '' },
        { key: 'sahip_ad', label: 'Adı Soyadı', get: (r) => r.sahip_ad || '' },
        { key: 'plaka', label: 'Plaka' },
        { key: 'giris', label: 'Giriş', get: (r) => new Date(r.giris).toLocaleString('tr-TR') },
        { key: 'cikis', label: 'Çıkış', get: (r) => (r.cikis ? new Date(r.cikis).toLocaleString('tr-TR') : 'İçeride') },
        { key: 'sure_dk', label: 'Süre (dk)', get: (r) => (r.sure_dk ?? '') },
      ]);
      downloadCSV(`daire_arac_${tarih}.csv`, csv);
    } else {
  ...
  async function exportPdf() {
    ...
    try {
      ...
      if (tab === 'dashboard') {
        ...
        const pdf = await newRaporPDF({ baslik: 'Daire-Araç Raporu', altBaslik: `Dönem: ${donem} • ${daireArac.length} kayıt` });
        pdf.addTable({
          head: [['Daire', 'Adı Soyadı', 'Plaka', 'Giriş', 'Çıkış', 'Süre']],
          body: daireArac.map((g) => [
            g.daire_no || '—',
            g.sahip_ad || '—',
            g.plaka,
            new Date(g.giris).toLocaleString('tr-TR'),
        ...
        pdf.save(`daire_arac_${tarih}.pdf`);
        ...
        <div className="bg-white dark:bg-slate-900 rounded-2xl shadow dark:shadow-black/30 border border-transparent dark:border-slate-800 overflow-x-auto">
          <table className="w-full text-base">
            ...
            <tbody>
              {daireArac.map((g) => (
                <tr key={g.id} className="border-t border-slate-100 dark:border-slate-800 text-slate-800 dark:text-slate-200">
                  <td className="p-3 font-mono font-semibold">{g.daire_no}</td>
                  <td className="p-3 hidden sm:table-cell">{g.sahip_ad || '—'}</td>
                  <td className="p-3 font-mono">{g.plaka}</td>
                  <td className="p-3 text-xs whitespace-nowrap">{new Date(g.giris).toLocaleString('tr-TR')}</td>
                  <td className="p-3 text-xs whitespace-nowrap">
                    {g.cikis
                      ? new Date(g.cikis).toLocaleString('tr-TR')
