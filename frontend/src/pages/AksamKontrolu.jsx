...
export default function AksamKontrolu() {
  ...
  const isYonetici = user?.rol === 'site_yonetici';
  const bildirimTelefonlari = user?.site?.bildirim_telefonlari || [];
  ...
  async function ozetGonder() {
    if (!bildirimTelefonlari.length) {
      toast.info('Önce "Yönetim Numaraları" ile bildirim numaralarını tanımlayın.');
      if (isYonetici) setNumaralarAcik(true);
      return;
    }
    if (!window.confirm(
      `Bugünün ihlalleri ${bildirimTelefonlari.length} yetkili numaraya WhatsApp ile gönderilsin mi?`
    )) return;
    setOzetBusy(true);
    try {
      const { data } = await api.post('/bildirimler/gunluk-ozet-gonder', {});
      if (data.ihlal_sayisi === 0) {
        toast.info('Bugün için çoklu araç ihlali yok, gönderilmedi.');
      } else {
        toast.success(
          `${data.ihlal_sayisi} ihlal özeti ${data.basari}/${data.alici_sayisi} numaraya gönderildi`
          + (data.mock ? ' (test/mock modu)' : '') + '.'
        );
      }
    } catch (e) { toast.error(apiError(e)); }
    finally { setOzetBusy(false); }
  }
  ...
  return (
    <div className="p-4 max-w-3xl mx-auto flex flex-col gap-4">
      ...
      <div className="bg-white dark:bg-slate-900 rounded-2xl shadow dark:shadow-black/30 border border-transparent dark:border-slate-800 p-4 flex flex-col gap-3">
        ...
        <Button size="lg" variant="success" onClick={ozetGonder} disabled={ozetBusy}>
          {ozetBusy ? 'Gönderiliyor…' : '📋 Günün İhlallerini Yönetime Gönder (WhatsApp)'}
        </Button>
        <div className="flex items-center justify-between gap-2 text-xs text-slate-500 dark:text-slate-400">
          <span>
            Yetkili numara: <span className="font-semibold tabular-nums">{bildirimTelefonlari.length}</span>/5
          </span>
          {isYonetici && (
            <button
              type="button"
              onClick={() => setNumaralarAcik(true)}
              className="font-medium text-brand-700 dark:text-brand-300 hover:underline"
            >
              ⚙️ Yönetim Numaraları
            </button>
          )}
        </div>
        {isBefore20() && (
...
function NumaralarModal({ mevcut, onClose, onSaved }) {
  const toast = useToast();
