...
export default function AksamKontrolu() {
  ...
  return (
    <div className="p-4 max-w-3xl mx-auto flex flex-col gap-4">
      <div>
        <h1 className="text-2xl font-bold">Akşam Kontrolü</h1>
        ...
        <>
          <div className="bg-white rounded-2xl shadow p-4 flex flex-col gap-3">
            <div className="flex justify-between items-center flex-wrap gap-2">
              <h2 className="text-lg font-semibold">İhlaller ({sonuc.ihlaller.length})</h2>
                  ...
                  <li key={i.daire_id} className="py-3 flex flex-col sm:flex-row sm:items-center gap-3">
                    <div className="flex-1">
                      ...
                      <div className="text-xs text-slate-500">
                        ...
                        {i.yeni_eklendi && <span className="ml-2 text-blue-600 font-semibold">YENİ</span>}
                      </div>
                      <div className="text-sm font-mono mt-1 flex flex-wrap gap-2">
                        {i.plakalar.map((p) => (
                          <span key={p} className="inline-flex items-center gap-1">
                            {p}
                            {i.misafir_plakalar?.includes(p) && (
                              <span className="font-sans text-[11px] bg-blue-50 text-blue-700 border border-blue-200 rounded px-1">
                                misafir
                              </span>
                            )}
                          </span>
            ...
            <div className="bg-white rounded-2xl shadow p-4">
              ...
              <div className="flex flex-wrap gap-2">
                {sonuc.kayitsiz_plakalar.map((p) => (
                  <span key={p} className="font-mono text-sm bg-amber-50 border border-amber-200 rounded px-2 py-1">
                    {p}
                  </span>
                ))}
              </div>
