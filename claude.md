...
**İki tamamlayıcı düzeltme (commit `3ae0713`):**
- **(1) Permütasyon kapısı** (`app.py` `_search_plate(items, allow_permute)` + `extract_plate`): permütasyon yalnız İRİ kutularda (`big`, `allow_permute=True` → `big/permuted` saha 22/0). `big+small` fallback'i `allow_permute=False` → küçük kutular (telefon/reklam/**"TR" ülke kodu**) permüte edilince geçerli FORMATTA ama semantik YANLIŞ plaka uyduruyordu (`all/permuted` 3/5 yanlış: 75CO51, 64IDE80, 23VCO46). Bulamazsa None → görevli elle / PR. `joined`+`single` (okuma sırası) korunur. **Reddedildi:** `all` yolunu tamamen kaldırmak (kullanıcı "sadece all/permuted" seçti — gerçekten küçük/uzak plakaları kaybetme riski). Test: `python_ocr/test_extract_plate.py` 5/5.
- **(2) Gövde-eşleştirme** (`plateMatcher.js` `plateBody` + `bodyMatchFromPlates` saf çekirdek + `findByBody` DB sarmalayıcı): OCR plaka GÖVDESİNİ okuyup il kodunu ("34") HİÇ yakalayamadığında (mavi AB şeridi küçük/kesik — saha kanıtı: NPE797/DEB464/FNE724 gövdeleri ham metinde var ama "34" yok → `findBestMatchFromRaw` skor<88 takılıyor). Gövdeyi kayıtlı listede **ends-with pencere skoruyla** eşleyip il kodunu kayıttan çıkarır. **Güvenlik (snapEligible dersi — kayıtsız misafiri kayıtlıya snaplemek ihlal gizler):** yalnız (a) gövde ayırt edici (**≥2 harf + ≥3 rakam**), (b) skor **≥90**, (c) **TEKİL** (farklı plakaya ait rakip skor `margin=8` içinde değil; belirsiz `657→34DT1657/34MEL657` null döner). `correctOCRGuess`'e **son çare** bağlandı (normal yollar `match`+`rawMatch` boşsa). Kaynak `body-registered` → `ocr_metrics.local_match_*`'a yazılır (sahada ölçülebilir). Test: `tests/plateMatcher.test.js` `bodyMatchFromPlates` describe 8/8.

**Köprü (zorunlu, `app.py`):** Permütasyon kapısı plaka=boş döndürünce `raw_text` de boşalıyordu (`best["raw"]` yalnız plaka kabul edilince doluyor) → gövde-eşleşme açlığa düşerdi. Fix: plaka kabul edilmese de **en zengin ham token seti** (`raw_fallback`, en çok karakterli pass) `raw_text`'te döndürülür. `kontroller.js:354` zaten `|| rawOcrText.length>=5` ile boş-plakada `correctOCRGuess` çağırıyor → akış bağlı. PR yolundan geçse bile (satır 407 `easySnapped`) gövde-eşleşme sonucu korunur.

**Net etki (akşamki 5 `all/permuted` vakası):** 3'ü "yanlış/elle"den **otomatik doğru**ya (75CO51→34NPE797, 64IDE80→34DEB464, 72ATR87→34FNE724), 2'si güvenli elle (23VCO46 çok bozuk skor<90; 65TTR12 belirsiz), **0 yanlış-snap**. Diğer 94 okuma bayt-bayt aynı. **`body-registered` PR-skip için güvenilir sayılmıyor** → PR yine çalışır (bu vakalar zaten PR'a gidiyordu, maliyet regresyonu yok); istenirse yüksek skorda PR-skip ayrı optimizasyon.

**İZLENECEK risk:** Gövde-eşleşme nadiren kayıtsız misafiri (gövdesi kayıtlıya çok benzerse) kayıtlıya snapleyebilir → güvenlikçi gözle yanlış görürse düzeltmeli. Yarın akşam batch'inden sonra `ocr_metrics` `body-registered` okumaları çekilip gerçek isabet + eşik (90) kalibre edilecek.

**DEPLOY DERSİ (kritik hata):** parktrack-backend deploy'u **kökten** yapılır (`parktrack2/fly.toml` + kök `Dockerfile`), `backend/`'den DEĞİL. parktrack-ocr `backend/python_ocr/fly.toml`'dan. Bu seansta çalışma-dizini kayması (`Set-Location backend` python_ocr içindeyken başarısız → orada kaldı) flyctl'i **OCR imajını parktrack-backend'e** deploy etmeye itti ("not listening on 0.0.0.0:5000" uyarısı = backend yanlış imaj). Kökten yeniden deploy ile düzeltildi. **Kural:** deploy öncesi `pwd`/`Get-Location` doğrula; backend = repo kökü.
